...
% [p,t]=distmesh2d(@dpoly,fh,0.001,[-1,-1; 2,1],pv,[-1,-1;-1,1;1,-1;1,1]);
...
line(p(:,1),p(:,2),'Marker','.','Color','r','LineStyle','none');
...
while 1
    ...
    line(barsmid(:,1),barsmid(:,2),hbars,'Marker','.','Color','r','LineStyle','none')
    ...
    line(p(ix,1),p(ix,2),'Marker','.','Color','g','LineStyle','none');
    outsidePoints = p(ix,:);
    ...
    for pix=1:length(outsidePoints)
        fpVec = [outsidePoints(pix,:);pnew(pix,:)];
        vectestH(pix) = line(fpVec(:,1),fpVec(:,2));
    end
